...
function Main() {
        ...
        function setRegularSectionHashes(withProjects) {
            setSectionHashes(
                [
                    'about-me-section',
                    'skills-section',
                    withProjects ? 'projects-section' : null,
                    'contact-section',
                ].filter(Boolean)
            );
        }
    ...
    function switchSectionInDirection(direction) {
        ...
        if (direction === 1 && activeSectionIndex === sectionHashes.length - 1)
            return;
    ...
    return (
        <div id='main-page-container'>
            ...
            <Nav
                linksComponent={
                    sectionHashes.length === 0 ? null : getLinksComponent()
                }
            />

            <div
                className={isLoaded ? 'fade-out-display-none' : 'fade-in-500ms'}
            >
                <LoadingScreen />
            </div>
            ...
            <div id='sections' className={isLoaded ? 'fade-in-500ms' : 'hide'}>
                <div
                    id='move-section-btns-container'
                    className='fixed-container'
                >
                    <button
                        className={
                            'general-btn-1 ' +
                            (isMoveSectionBtnDisabled(-1) ? 'disabled' : '')
                        }
                        id='move-section-up-btn'
                        onClick={() => switchSectionInDirection(-1)}
                    >
